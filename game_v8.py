...
def barrier():
    ...
    if gaming:
        barrier_x -= speed
    barrier_surface = [cactus0, cactus1, cactus2, cactus3, cactus4][which_barrier]
    if which_barrier == 0:
        barrier_y = 193
        ...
        barrier_y = 168
    barrier_rect = barrier_surface.get_rect(topleft=(barrier_x, barrier_y))
    barrier_mask = p.mask.from_surface(barrier_surface)
    offset = (barrier_rect.x - dino_x, barrier_rect.y - dino_y)
    if dino_mask.overlap(barrier_mask, offset):
        gaming = False
    screen.blit(barrier_surface, barrier_rect.topleft)
    if barrier_x < -96:
        barrier_x = 600
        which_barrier = random.randint(0, 4)
        speed += 0.01
...
def reset():
    global cloud_position_list, speed, dino_x, dino_y, jump_speed, is_jumping, count, count1, which_barrier, barrier_x, gaming, score, restart_delay, is_day
    score = 0
    cloud_position_list = [[600, random.randint(20, 150)], [600 + random.randint(100, 400), random.randint(20, 150)], [850 + random.randint(100, 400), random.randint(20, 150)]]
    speed = 1
    dino_x = 80
...
def invert_all():
    global bg, cloud_image, game_over_image, HI, restart, cactus0, cactus1, cactus2, cactus3, cactus4, dino_list
    bg = invert_colors(bg)
    cloud_image = invert_colors(cloud_image)
    game_over_image = invert_colors(game_over_image)
    HI = invert_colors(HI)
    restart = invert_colors(restart)
    cactus0 = invert_colors(cactus0)
    cactus1 = invert_colors(cactus1)
    cactus2 = invert_colors(cactus2)
    cactus3 = invert_colors(cactus3)
    cactus4 = invert_colors(cactus4)
    for d in range(0, 4):
        dino_list[d] = invert_colors(dino_list[d])
    for n in range(0, 10):
        bright_number_list[n] = invert_colors(bright_number_list[n])
        dark_number_list[n] = invert_colors(dark_number_list[n])


def day_night():
    global score, switch_cooldown, day_night_switch_state, is_day, bg_colour, bg, cloud_image, game_over_image, HI, restart, cactus0, cactus1, cactus2, cactus3, cactus4, dino_list
    if int(score) > 0 and int(score) % 100 == 0 and switch_cooldown > 100:
        is_day = not is_day
        switch_cooldown = 0
    switch_cooldown += 1
    if is_day:
