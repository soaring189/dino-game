...
llama0 = p.transform.scale(p.transform.flip(p.image.load("./Assets/Llama.png").convert_alpha(), True, False), (50, 50))
llama1 = p.transform.scale(p.transform.flip(p.image.load("./Assets/Llama2.png").convert_alpha(), True, False), (50, 50))
llama2 = p.transform.scale(p.transform.flip(p.image.load("./Assets/Llama3.png").convert_alpha(), True, False), (50, 50))
...
pterosaur0_day, pterosaur0_night, pterosaur1_day, pterosaur1_night = None, None, None, None
llama0_day, llama0_night, llama1_day, llama1_night, llama2_day, llama2_night = None, None, None, None, None, None
...
def barrier():
    global which_barrier, cactus_list, barrier_x, speed, gaming, animal_state, pterosaur_y, llama_y, llama_jump
    if gaming:
        barrier_x -= speed
        if which_barrier == 5:
            animal_state += 0.002
        elif which_barrier == 6:
            animal_state += 0.01
    if which_barrier == 0:
        barrier_y = 193
    ...
    if which_barrier in [0, 1, 2, 3, 4]:
        ...
        if llama_y < 179:
            barrier_rect = llama0.get_rect(topleft=(barrier_x, barrier_y))
            barrier_mask = p.mask.from_surface(llama0)
            offset = (barrier_rect.x - dino_x, barrier_rect.y - dino_y)
            screen.blit(llama0, (barrier_x, barrier_y))
        else:
            if int(animal_state) % 2 == 0:
                barrier_rect = llama1.get_rect(topleft=(barrier_x, barrier_y))
                barrier_mask = p.mask.from_surface(llama1)
                offset = (barrier_rect.x - dino_x, barrier_rect.y - dino_y)
                screen.blit(llama1, (barrier_x, barrier_y))
            else:
                barrier_rect = llama2.get_rect(topleft=(barrier_x, barrier_y))
                barrier_mask = p.mask.from_surface(llama2)
                offset = (barrier_rect.x - dino_x, barrier_rect.y - dino_y)
                screen.blit(llama2, (barrier_x, barrier_y))
        if gaming:
            if barrier_x <= 160 and llama_y <= 181:
                llama_y -= llama_jump
                llama_jump -= 0.01
    ...
    if barrier_x < -96:
        ...
        llama_y = 180
        llama_jump = 1
        which_barrier = random.randint(0, 6)
        if speed < 1.5:
            speed += 0.01
...
def precompute_image():
    global bg, cloud_image, game_over_image, HI, restart, cactus0, cactus1, cactus2, cactus3, cactus4, pterosaur0_day, pterosaur0_night, pterosaur1_day, pterosaur1_night, llama0_day, llama0_night, llama1_day, llama1_night, llama2_day, llama2_night, dino_list, bright_number_list, dark_number_list, bg_day, bg_night, cloud_day, cloud_night, game_over_day, game_over_night, HI_day, HI_night, restart_day, restart_night, cactus_day, cactus_night, dino_day, dino_night, bright_number_list_day, bright_number_list_night, dark_number_list_day, dark_number_list_night
    ...
    llama0_day, llama0_night = llama0.copy(), invert_colors(llama0)
    llama1_day, llama1_night = llama1.copy(), invert_colors(llama1)
    llama2_day, llama2_night = llama2.copy(), invert_colors(llama2)
...
def day_night():
    global score, switch_cooldown, day_night_switch_state, is_day, moonX, bg_colour, bg, cloud_image, game_over_image, HI, restart, cactus_list, pterosaur0, pterosaur1, llama0, llama1, llama2, dino_list, bright_number_list, dark_number_list
    ...
    llama0 = llama0_day if is_day else llama0_night
    llama1 = llama1_day if is_day else llama1_night
    llama2 = llama2_day if is_day else llama2_night
